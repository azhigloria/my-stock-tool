...
with st.sidebar:
    st.header("🔍 配置")
    user_input = st.text_input("请输入最多3支股票代码，用英文逗号区隔", "600519, 002028")
    analyze_btn = st.button("🚀 启动PK")
...
if analyze_btn:
    ...
    if raw_results:
        with st.spinner("擂台搭建中..."):
            ...
            ### 报告框架：
            一、公司画像与核心竞争力：使用Markdown表格 [公司 | 核心标签 | 核心优势（护城河）]。
            二、深度对比分析：对比各家，列出 [✅机会点] 和 [❌风险点]（每项限20字）。
            三、理性选择建议：根据不同的投资画像给出股票推荐，并给出理由。
